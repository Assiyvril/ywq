...
def check_file(filename):
    ...
    # print('如果要自定义开始的行数，请输入行数后回车，否则直接回车')
    # try:
    #     start_line_num_input = int(input())
    # except ValueError:
    #     raise ValueError('输入的行数不是整数')
...
def draw():
    ...
    ax.xaxis.get_offset_text().set_fontsize(50)
    plt.tick_params(labelsize=60, labelcolor='green')
    plt.plot(x_list[-400::2], y_list[-400::2], linewidth=5)
    plt.xlabel('Step', fontsize=55, fontweight='bold', color='red')
    plt.ylabel('TotEng', fontsize=55, fontweight='bold', color='red')
    plt.title('YWQ_last200', fontsize=88)
    plt.savefig('ywq_last200.png')
    return True
